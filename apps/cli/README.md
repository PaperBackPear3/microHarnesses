...
`@micro-harnesses/*` packages into a runnable CLI.
...
- Tooling and behavior extensions: plugin packages (for example
  `@micro-harnesses/plugin-plan-mode`, `@micro-harnesses/plugin-subagents`)
...
| Plugin | What it adds |
|---|---|
| `builtInProviderPlugins()` from `@micro-harnesses/providers` | Providers: `openai`, `anthropic`, `ollama` |
