...
export async function loadCliConfig(
  ...
  const envStateDir =
    process.env.MH_STATE_DIR || process.env.MICRO_HARNESS_STATE_DIR
  const envProvider =
    process.env.MH_PROVIDER || process.env.MICRO_HARNESS_PROVIDER
  const envModel = process.env.MH_MODEL || process.env.MICRO_HARNESS_MODEL
  const envEffort = parseEffort(
    process.env.MH_EFFORT || process.env.MICRO_HARNESS_EFFORT,
  )
  const envMode = parseMode(
    process.env.MH_MODE || process.env.MICRO_HARNESS_MODE,
  )
  ...
  return {
    stateDir:
      ...
      defaults.stateDir,
    promptsDir:
      overrides.promptsDir ?? fromFile.promptsDir ?? defaults.promptsDir,
    provider:
      overrides.provider ??
      envProvider ??
      fromFile.provider ??
    ...
    effort:
      overrides.effort ??
      envEffort ??
      parseEffort(fromFile.effort) ??
      defaults.effort,
    mode:
      overrides.mode ?? envMode ?? parseMode(fromFile.mode) ?? defaults.mode,
    maxIterations:
    ...
    privacyMode:
      overrides.privacyMode ?? fromFile.privacyMode ?? defaults.privacyMode,
